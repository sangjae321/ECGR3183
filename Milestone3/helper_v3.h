...
///MILESTONE 1:
    ...
    vector<string> interpret(string input_text_file){
    ...
    vector<double> setMem(string input_text_file){
        ...
        if(input.is_open()){
            while(getline(input, line)){
                while(line[0] == '<'){
                    string temp = line.substr(1, line.find('>')-1);
                    mem.push_back(stod(temp));
                    line = line.substr(line.find('>')+1);
                    temp = line.substr(1, line.find('>')-1);
                    mem.push_back(stod(temp));
                    line = line.substr(line.find('>')+1);
                }
            }
    ...
    vector<string> decode(string input_txt_file){
        vector<string> instructions;
...
void execute(vector<string> a, vector<double> mem){
        int vec_count = 0;
        ...
        while(vec_count < a.size()){
            ...
            ///Load Instruction
                ...
                vec_count++;
                double temp;
                double toFind = ieee2decimal(reg_file[binary2decimal(a[vec_count+1])]);
                for(int i = 0; i < mem.size(); i=i+2){
                    if(toFind == mem[i]){
                        temp = mem[i+1];
                    }
                }
                reg_file[binary2decimal(a[vec_count])] = decimal2ieee(temp);
                vec_count += 2;
            }
            ...
            ///Store Instruction
                ...
                cout << "STORE" << endl;
                vec_count++;
                double loc = ieee2decimal(reg_file[binary2decimal(a[vec_count])]);
                double temp = ieee2decimal(reg_file[binary2decimal(a[vec_count+1])]);
                mem.push_back(loc);
                mem.push_back(temp);
                vec_count += 2;
            }
